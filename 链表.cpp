...
//定义链表结构体变量:
typedeftypedef struct node {struct node {
	ElemType data;ElemType data;
	struct node* next;struct node* next;
}Node}节点;

//链表的动态内存分配和初始化:
NodeNode * initList() {initList() {
	Node* head = (Node*)malloc(sizeof(Node));Node* head = (Node*)malloc(sizeof(Node));
	//判断链表head是否为空://判断链表head是否为空:
	if (head == NULL) {if (head == NULL) {
		//链表head为NULL,初始化失败,返回NULL://链表head为NULL,初始化失败,返回NULL:
...
//链表的头插法:
intint insertHead(Node* L, ElemType e) {insertHead(Node* L, ElemType e) {
	//判断L是否为空://判断L是否为空:
	if (L == NULL) {if (L == NULL) {
		//链表L为空,插入失败;//链表L为空,插入失败;
		return NULL;return NULL;
	}}
	//创建一个新的节点,并动态分配内存;//创建一个新的节点,并动态分配内存;
